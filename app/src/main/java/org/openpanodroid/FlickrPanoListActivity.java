...
public class FlickrPanoListActivity extends ListActivity implements OnItemClickListener {
            ...
            if (tryCnt >= MAX_TRY_CNT) {
                ...
                if (resultCnt == 0) {
                    ...
                    if (imgInfos.size() == 0) {
                        ...
                        waitSearchDialog.dismiss();
                        UIUtilities.showAlert(FlickrPanoListActivity.this,
                                null,
                                getString(R.string.noImages),
                                new AlertListener());
                    } else {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        waitSearchDialog.setButton(ProgressDialog.BUTTON_NEGATIVE,
    ...
    private FlickrPhotoQuery createPhotoQuery() {
        ...
        FlickrPhotoQuery query =
                new FlickrPhotoQuery(queryMsgHandler, searchTags, sortCritertia, currentPage, RESULTS_PER_PAGE);
    ...
    private void filterImageInfos(List<FlickrImageInfo> list) {
        for (Iterator<FlickrImageInfo> it = list.iterator(); it.hasNext(); ) {
            ...
            if (!searchParameters.includeLowResImages &&
                    (imageInfo.originalSize == null || imageInfo.originalSize.width < HIGH_QUALITY_MIN_WIDTH)) {
